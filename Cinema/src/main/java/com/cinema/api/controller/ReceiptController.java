...
public class ReceiptController {
    ...
    private Long getCurrentAdminId() {
        // TODO: получить из SecurityContextHolder после настройки JWT
        return 1L;
    }
    ...
    public ResponseEntity<ReceiptRs> createDraft() {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(receiptService.createReceipt(getCurrentAdminId()));
    }
    ...
    public ResponseEntity<ReceiptRs> sell(@PathVariable Long receiptId,
        ...
        return ResponseEntity.ok(receiptService.sell(receiptId, getCurrentAdminId(), paymentMethod));
    }

    @PostMapping("/{receiptId}/cancel")
    public ResponseEntity<ReceiptRs> cancel(@PathVariable Long receiptId) {
        return ResponseEntity.ok(receiptService.cancel(receiptId, getCurrentAdminId()));
    }
